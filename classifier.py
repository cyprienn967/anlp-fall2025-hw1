...
class LlamaZeroShotClassifier(torch.nn.Module):
    ...
    def forward(self, input_ids):
        ...
        for i, label_token_ids in enumerate(self.label_name_ids):
            total_log_prob = torch.sum(log_probabilities[:, :, label_token_ids], dim=-1)
...
class LlamaEmbeddingClassifier(torch.nn.Module):
    ...
    def forward(self, input_ids):
        ...
        '''
        _, hidden = self.llama(input_ids)
        fidden = hidden[:, -1, :]
        fidden = self.dropout(fidden)
        logits = self.classifier_head(fidden)
